...
int64_t Problem_013::solve()
{
    ...
    for (std::string number : data)
        sum += stringToBigInteger(number);

    int64_t result = 0;
    for (int digit : utils::bigint::get_first_n_digits(sum, 10))
...
{
    int64_t sum = 0;    
    for (int digit : utils::bigint::get_digits(utils::bigint::pow(2, 1000)))
        sum += digit;
...
int64_t Problem_020::solve()
{
    ...
    int64_t sum = 0;
    for (int digit : utils::bigint::get_digits(utils::number::get_factorial(100)))
        sum += digit;
